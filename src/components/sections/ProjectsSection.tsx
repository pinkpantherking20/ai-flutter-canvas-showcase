...
const projects = [{
  ...
  // Note: You might need to update this image file
      ...
      <Tabs defaultValue="all" className="mb-12">
        <div className="flex justify-center">
          <TabsList className="mb-8">
            {categories.map(category => <TabsTrigger key={category} value={category} className="capitalize">
        ...
        {categories.map(category => <TabsContent key={category} value={category} className="mt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(category === 'all' ? projects : projects.filter(p => p.category === category)).map(project => <ProjectCard key={project.id} project={project} />)}
            </div>
          </TabsContent>)}
      </Tabs>
...
const ProjectCard = ({
  ...
  return <Card className="overflow-hidden card-hover border-border/50 bg-card/50 backdrop-blur-sm">
      ...
      <CardContent className="pt-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.map(tag => <Badge key={tag} variant="outline" className="bg-secondary/30">
              {tag}
            </Badge>)}
        </div>
      </CardContent>
      <CardFooter className="flex justify-between pt-0">
        <Button variant="ghost" size="sm" className="gap-2" onClick={() => window.open(project.githubLink, '_blank')}>
          <Github className="h-4 w-4" />
          Code
        </Button>
        {project.liveLink && project.liveLink !== '#' && <Button variant="ghost" size="sm" className="gap-2" onClick={() => window.open(project.liveLink, '_blank')}>
